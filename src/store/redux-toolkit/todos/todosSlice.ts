...
const computedPage = (state: TodosState) => {
  ...
  if (
    state.todos.slice(startIndex, startIndex + state.itemsPerPage).length ===
      0 &&
    state.currentPage > 1
  ) {
    state.currentPage -= 1;
...
const handleRejectError = (
  state: TodosState,
  action: RejectedAction<ApiError>,
  message: string
) => {
  if (action.payload) {
...
>('todos/createTodo', async (todo: NewTodo, { rejectWithValue }) => {
  try {
    const response = await fetch(API_URL, {
      method: 'POST',
      headers: {
...
>('todos/updateTodo', async (todo: Todo, { rejectWithValue }) => {
  try {
...
>('todos/deleteTodo', async (id: string, { rejectWithValue }) => {
  try {
...
>('deleteCompletedTodos', async (_, { getState, rejectWithValue }) => {
  try {
...
>('todos/deleteAllTodos', async (_, { rejectWithValue }) => {
  try {
    const response = await fetch(API_URL);
    const todos = await handleFetchError(
      response,
...
export const updateTodoOrder = createAsyncThunk<
  ...
  { rejectValue: ApiError }
>('todos/updateTodoOrder', async ({ id, order }, { rejectWithValue }) => {
  try {
    const response = await fetch(`${API_URL}/${id}`, {
      method: 'PUT',
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(deleteCompletedTodos.fulfilled, (state, action) => {
        ...
        ); // Удаляем задачи айди которых есть в возвращённом массиве
        computedPage(state);
      })
...
export const { setCurrentPage, setItemsPerPage, reorderTodos, setDraggedTodo } =
  todosSlice.actions;
